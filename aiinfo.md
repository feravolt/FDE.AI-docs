...
Одним из этих методов является автоматическое занижение частоты ЦП (только большого кластера) на ~20% когда ИИ решает переключиться на режим энергосбережения (что уменьшит энергопотребление в ущерб производительности) и повышает частоту обратно до макс. в других режимах. Так же будут автомачески изменены некоторые настройки приложения.
...
- Переключать опцию закрытия фоновых приложений при выключенном экране `/killbgapps` | `/killbgapps off`
- Переключать работу датчиков/сенсоров устройства в режиме Doze `/disablesensorsondoze` | `/disablesensorsondoze off`
- Переключать опцию более глубокой оптимизации системы в реальном времени `/advancedai` | `/advancedai off`
- Переключать опцию автоматической смены частоты экрана если поддерживается `/autorefreshrate` | `/autorefreshrate off`
- Переключать опцию автоматической предзагрузки тяжелых приложений/игр если установлено приложение Preload Pro `/autopreload` | `/autopreload off`
- Копировать текущий лог FDE.AI в корень внутренней памяти как текстовый файл `/dumpfdelog`
- Переключать вибрацию при выполнении FDE.AI `/vibro off` | `/vibro`
- Переключать применение всех основных твиков FDE.AI (можно оставить только работу ИИ/турбо) `/alltweaks off` | `/alltweaks`
...
- Менять поведение тротлинга во время зарядки устройства `/ignorethrottling` | `/ignorethrottling off`
- Активировать режим отладки программы `/fdebug` | `/fdebug off`
