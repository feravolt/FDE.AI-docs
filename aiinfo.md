...
Чтобы сбросить даунскейл используйте команду '/resetscale' в чате ИИ. Так же доступна опция теста даунскейла через чат-команду (см. список команд ниже). Если вдруг что-то пошло не так, изменения можно сбросить вручную командой ```wm size reset && wm density reset``` в консоли/терминале.
...
- Дополнительно оптимизировать систему автоматически каждый день, примерно в 05:00 если устройство заряжается и экран выключен `/extraopt auto` | `/extraopt noauto`
